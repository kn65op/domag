...
suspend fun AppDatabase.consumeItem(itemId: Int, consumedAmount: FixedPointNumber) {
    ...
    val consume =
        Consume(amount = consumedAmount, categoryId = item.categoryId, date = ZonedDateTime.now())
    consumeDao().insert(consume)

}

private fun Item.consumeItem(consumedAmount: FixedPointNumber) =
    Item(uid, amount - consumedAmount, depotId, categoryId, description, bestBefore)
